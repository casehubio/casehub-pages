...
const ZONES: Array<{ zone: SnapZone; label: string; col: number; row: number; colSpan?: number; rowSpan?: number }> = [
    ...
    for (const z of ZONES) {
      ...
      cell.addEventListener("click", (e) => {
        ...
        if (frame.snappedZone === z.zone) {
          ...
          engine.snapFrame(frameKey, z.zone, canvasSize);
          container.dispatchEvent(new CustomEvent("pages-frame-snap", {
            bubbles: true, composed: true, detail: { frameKey, zone: z.zone },
          }));
        }
        closeDropdown();
      });
