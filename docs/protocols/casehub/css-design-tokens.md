...
theme-portable styling. Role tokens reference primitives via `var()`:
...
`applyTheme(name)` sets the class and injects the pre-built CSS.
Multiple themes can coexist via class scoping on different subtrees.
